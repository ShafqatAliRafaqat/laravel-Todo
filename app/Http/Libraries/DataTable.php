...
class DataTable{
    ...
    private static function groupActions() {
        $customActionType = Input::get('customActionType');
        $customActionName = Input::get('customActionName');
        $customActionId = Input::get('customActionId', []);
    ...
    public static function get($where = array(), $joins = array(), $extraColumns = array(), $misc = array()) {
        self::$_start = intval(Input::get('start', 0));
        self::$_length = intval(Input::get('length', 10));
        self::$_order = Input::get('order');
        self::$_requestColumns = Input::get('columns');
        self::$_search = Input::get('search');
        self::$_draw = Input::get('draw', 1);
        ...
        $queryBeforeLimit = self::$_query; //(isset(self::$_miscellaneous['groupBy'])) ? self::$_query->groupBy(self::$_miscellaneous['groupBy']):self::$_query->groupBy(self::$_table.'.'.self::$_primaryKey);
    ...
    public static function whereHas($relation, $condition) {
